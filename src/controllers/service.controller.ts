...
export const getServiceDataController = ApiHandler(
  async (req: Request, res: Response) => {
    ...
    if (service.integrationType === "sheets") {
      const sheetData = await getSpreadsheetDataFromServiceId(service._id);
...
export const getGuildsOfUserController = ApiHandler(
  async (req: Request, res: Response) => {
    const guilds = await getGuilds(req.customer.accessToken);
    const responseData = guilds.map((guild) => {
      return {
        id: guild.id,
        name: guild.name,
        icon: `https://cdn.discordapp.com/icons/${guild.id}/${guild.icon}.webp`,
...
export const generateBotInviteLinkController = ApiHandler(
  async (
...
export const createServiceController = ApiHandler(
  async (
...
export const createTMServiceController = ApiHandler(
  async (
